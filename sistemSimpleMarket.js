...
class Report {
    constructor(_idReport, _namaProduk, _qty, _price) {
...
printDataCart = () => {
    ...
    printReport = () => {
        ...
        dbReport.forEach((item, index) => {
            ...
            qtyomzet += item.qty
            omzet += item.priceTotal
        })
        document.getElementById("report-page").style.display = "block";
        document.getElementById("listReport").innerHTML = tableElement;
        document.getElementById("qty-page").innerHTML = 'Total Barang Terjual : ' + qtyomzet;
        document.getElementById("omzet-page").innerHTML = 'Total Pemasukan : ' + omzet.toLocaleString();
    } 
...
editCart = (index) => {
    ...
    while (true) {
        ...
        if (dbProduk[noProduk].stock - (qtyProduk - dbUser[userLogin].keranjang[index].qty) < 0) {
            alert('Qty anda Berlebihan')
            if (!confirm("Edit lagi Qty Anda?")) {
                break
            } 
        } else {
...
deleteCart = (index) => {
    ...
    if (dbUser[userLogin].keranjang[index].qty == 1) {
        dbProduk[noProduk].stock += dbUser[userLogin].keranjang[index].qty
        dbUser[userLogin].keranjang.splice(index, 1);
    } else {
        dbProduk[noProduk].stock++
        dbUser[userLogin].keranjang[index].qty--
        dbUser[userLogin].keranjang[index].priceTotal = dbUser[userLogin].keranjang[index].price * dbUser[userLogin].keranjang[index].qty
...
bayar = () => {
    while (true) {
        ...
        if (uangPembayaran < totalBayar) {
            ...
            for (let i = 0; i < dbUser[userLogin].keranjang.length; i++) {
                dbReport.push(new Report(dbUser[userLogin].keranjang[i].id, dbUser[userLogin].keranjang[i].namaProduk, dbUser[userLogin].keranjang[i].qty, dbUser[userLogin].keranjang[i].price))
            }
            console.table(dbReport)
